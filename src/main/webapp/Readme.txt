###############################################################################################
##
#  PHP営業日カレンダー【Calendar01】　簡易版　ver1.0.0 (2013.09.4公開)
#
#  営業日カレンダープログラムです。
...
・フリー（無料）のPHPプログラムです。もちろん商用サイトでもどんなサイトでも設置OKです。

・管理画面から登録、編集できるので完全自動化（CMS）として運用可能です。

・スマホ（スマートフォン）、携帯用に最適化したファイルも同梱しています（サンプル参照）。
　3デバイス同時更新が可能です。
　スマホ、ガラケーは表形式ではなく縦長のリスト形式で表示されます。
...
・カレンダーは何ヶ月後まで表示させるか、前月を表示するかなどの設定が可能です。
...
ブラウザで「あなたのサイトのURL」/calendar01/admin.phpを表示し、管理画面にログインし、登録を行ってみてください。正常に登録、編集できればOKです。定休日設定も確認下さい。
...
http://www.php-factory.net/calendar/01.php
